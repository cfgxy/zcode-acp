...
- **Node.js ≥ 22**（桥接层用 `node:sqlite` 同步 tasks-index；ZCode CLI
  运行时也要求 Node ≥ 22）
- **仅支持 Linux**，并且本机正在运行 ZCode Desktop
- 已安装 `zcode` CLI 并位于 `PATH` 上（或通过 `ZCODE_BIN` 指定）
- ZCode 凭证位于 `~/.zcode/v2/config.json`（由 ZCode App 创建）

首次使用以及每次后端重启时，bridge 会在配置缺失或过期后自动重新捕获桌面附着配置；也可手动刷新：

```bash
zcode-acp profile refresh
```

配置写入 `$XDG_CONFIG_HOME/zcode-acp/desktop-profile.json`（未设置时为
`~/.config/zcode-acp/desktop-profile.json`）。无效配置和非 Linux 平台会直接安全失败，
不会回退到旧的全量环境继承方式。
...
| `ZCODE_BASE_URL`                   | _（Desktop profile）_ | 由 `profile refresh` 从当前 ZCode Desktop 进程的非敏感白名单环境捕获                                                                                                                                                                                                                |
...
| 磁盘 | 写入 XDG 配置目录中的 `desktop-profile.json`（0600，目录 0700，保留 `.bak`）以及已存在的 `~/.zcode/v2/tasks-index.sqlite`。profile 只含固定非敏感白名单，绝不保存 workspace identity、token、API key、cookie、authorization、password、secret、credential 或 `SSH_*` |
